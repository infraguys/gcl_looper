...
class TestPostgresTableLockDriver:
    def test_acquire_writes_the_row(self, fake_pg):
        ...
        assert "expires_at timestamptz" in create_sql
        assert "ON CONFLICT (name) DO NOTHING" in ensure_sql
        assert ensure_params == ("master_key",)
        assert "worker_id = %s" in acquire_sql
        assert "expires_at = now() + make_interval" in acquire_sql
        assert "expires_at < now()" in acquire_sql
    ...
    def test_fork_forces_new_connection_and_worker_identity(self, fake_pg, monkeypatch):
        first = FakeConnection(script=[-1, 0, 1])
        second = FakeConnection(script=[-1, 0, 1])
        fake_pg.connect_script.extend([first, second])
        driver = make_driver()
        assert driver.acquire() is True
        parent_worker_id = driver.worker_id

        child_pid = driver._worker_pid + 1
        monkeypatch.setattr(
            "gcl_looper.election.drivers.table.os.getpid", lambda: child_pid
        )
        assert driver.acquire() is True
        assert driver.worker_id != parent_worker_id
        assert first.closed is False
        assert len(second.statements) == 3  # create + ensure + acquire
        assert second.statements[-1][1][0] == driver.worker_id
    ...
    def test_lock_timeout_property(self, fake_pg):
        driver = make_driver(lock_timeout=7)
        assert driver.lock_timeout == 7
