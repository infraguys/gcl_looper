...
from gcl_looper.election.drivers.pg_advisory import PostgresAdvisoryLockDriver
...
class TestPostgresTableLock:
    ...
    def test_owner_controls_lease_expiration(self):
        key = unique_key("table_owner_timeout")
        owner = make_table_driver(key, lock_timeout=3)
        contender = make_table_driver(key, lock_timeout=1)
        try:
    ...
    def test_missing_table_without_autocreate(self):
        ...
        try:
            with pytest.raises(election_exc.BackendError):
                driver.acquire()
        finally:
            driver.close()
...
class ElectionService(basic_service.BasicService):
    __test__ = False

    def __init__(self, **kwargs):
        super(ElectionService, self).__init__(iter_min_period=0, iter_pause=0, **kwargs)
        self.iterations = 0
        self.became = 0
        self.lost = 0
...
class TestServiceMasterElection:
    def test_service_skips_iterations_until_it_becomes_master(self):
        ...
        service_elector = DbLeaderElector(
            make_table_driver(key, lock_timeout=2), refresh_interval=0.5
        )
        service = ElectionService(elector=service_elector)
        try:
            for _ in range(3):
    ...
    def test_service_demoted_when_master_lock_taken_over(self):
        ...
        try:
            ...
            thief._ensure_ready()
            thief._execute(
                "UPDATE {table} SET worker_id = %s, locked_at = now() "
                "WHERE name = %s".format(table=thief.table_name),
                (thief.worker_id, key),
            )
            time.sleep(0.6)
            service._loop_iteration()
