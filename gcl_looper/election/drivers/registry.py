...
#:
#: 1. implement a :class:`~gcl_looper.election.drivers.base.BaseLockDriver`
#:    subclass (for the table-based locks it is usually enough to subclass
#:    :class:`~gcl_looper.election.drivers.table.TableLockDriver` and override
...
BUILTIN_DRIVERS: tp.Dict[str, str] = {
    "postgres_advisory": (
        "gcl_looper.election.drivers.pg_advisory:PostgresAdvisoryLockDriver"
    ),
    "postgres_table": "gcl_looper.election.drivers.pg_table:PostgresTableLockDriver",
}
...
def get_driver_class(name: str) -> tp.Type[drivers_base.BaseLockDriver]:
    ...
    if not (
        ...
        and issubclass(driver_class, drivers_base.BaseLockDriver)
    ):
        raise TypeError(
...
def create_driver(name: str, **kwargs: tp.Any) -> drivers_base.BaseLockDriver:
