...
Backend of :class:`~gcl_looper.election.drivers.table.TableLockDriver` for
...
:class:`~gcl_looper.election.drivers.table.TableLockDriver` and provide the
...
from gcl_looper.election.drivers import table as table_base
...
class PostgresTableLockDriver(table_base.TableLockDriver):
    ...
    def _connect(self) -> tp.Any:
        ...
        try:
            ...
            raise exc.BackendError(reason=repr(e)) from e
    ...
    def _sql_acquire(self) -> str:
        ...
        return (
            "UPDATE {table} "
            "SET worker_id = %s, locked_at = now(), "
            "expires_at = now() + make_interval(secs => %s), updated = now() "
            "WHERE name = %s AND ("
            "worker_id IS NULL "
            "OR worker_id = %s "
    ...
    def _sql_release(self) -> str:
        return (
            "UPDATE {table} "
            "SET worker_id = NULL, locked_at = NULL, expires_at = NULL, "
            "updated = now() WHERE name = %s AND worker_id = %s"
        ).format(table=self.table_name)

    def _acquire_params(self) -> tp.Tuple[tp.Any, ...]:
        # Matches the placeholder order in `_sql_acquire`:
        # worker_id, lock_timeout, name, worker_id (self-refresh).
        return (
            self.worker_id,
