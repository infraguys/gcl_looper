...
unique ``worker_id`` and an owner-defined expiration timestamp into the row
with a single *atomic* ``UPDATE`` that only succeeds when the row is free,
expired, or already owned by this very worker.
...
(see :class:`gcl_looper.election.drivers.pg_table.PostgresTableLockDriver`).
"""
...
class TableLockDriver(drivers_base.BaseLockDriver, abc.ABC):
    """Generic table-based distributed lock.

    Args:
        connection_url: DSN/connection URL of the database.
        lock_key: Logical lock (row) name.
        lock_timeout: Seconds after which this worker's unrefreshed lock is
            considered abandoned and can be taken over by another worker. It
            must be clearly greater than the election refresh interval.
    ...
    def __init__(
        ...
        self._table_name = drivers_base.validate_identifier(table_name)
    ...
    def lock_timeout(self) -> float:
        """Seconds after which an unrefreshed lock can be taken over."""
    ...
    def _execute(self, sql: str, params: tp.Sequence[tp.Any]) -> int:
        ...
        :class:`~gcl_looper.election.exceptions.BackendError`.
        """
        raise NotImplementedError()
    ...
    def _acquire_params(self) -> tp.Tuple[tp.Any, ...]:
        ...
        the lock timeout, the lock key and the worker id (self refresh).
        """
        return (self._worker_id, self._lock_timeout, self._lock_key, self._worker_id)
